...
make verify_mongo
```
4) Для построения булевого индекса (таблица postings.csv) и получения данных для Ципфа (таблица zipf.csv) необходимо запустить сервис builder 
```
make builder
```
5) Для построения графика распределения частот по Ципфу необходимо запустить сервис zipf
```
make zipf
```
6) Для запуска терминала булевого поиска по документам необходимо запустить сервис searcher
```
make searcher
```